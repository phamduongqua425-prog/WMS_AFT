...
import { deleteExportSession, updateExportNote } from '../actions'
import { Trash2, Pencil, Check, X } from 'lucide-react'

export function ExportSessionActions({ sessionId, note }: { sessionId: string; note?: string | null }) {
  ...
  function handleSaveNote() {
    startTransition(async () => {
      await updateExportNote(sessionId, noteVal)
      setEditing(false)
    })
  }
  ...
  function handleDelete() {
    startTransition(async () => {
      await deleteExportSession(sessionId)
    })
  }
