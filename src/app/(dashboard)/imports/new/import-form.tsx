...
export function ImportForm({ branches, products }: { branches: Branch[]; products: Product[] }) {
  ...
  const [items, setItems] = useState<LineItem[]>([
    { id: ++lineId, product_id: '', quantity: '', expiry_date: '', batch_code: '' },
  ])

  function addRow() {
    setItems(prev => [...prev, { id: ++lineId, product_id: '', quantity: '', expiry_date: '', batch_code: '' }])
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!branchId) { setError('Vui lòng chọn điểm nhập hàng'); return }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-gray-700">
              Điểm nhập hàng <span className="text-red-500">*</span>
            </label>
            <select
              value={branchId}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        ...
        <p className="text-xs text-gray-400">
          Hạn sử dụng quan trọng — hệ thống sẽ tự cảnh báo khi hàng cận date.
        </p>
      </div>
